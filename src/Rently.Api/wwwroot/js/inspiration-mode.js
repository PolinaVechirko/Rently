...
function initInspirationMode() {
  ...
  if (
    params.has("checkin") &&
    document.getElementById("search-checkin")._flatpickr
  ) {
    document
      .getElementById("search-checkin")
      ._flatpickr.setDate(params.get("checkin"));
  }
  if (
    params.has("checkout") &&
    document.getElementById("search-checkout")._flatpickr
  ) {
    document
      .getElementById("search-checkout")
      ._flatpickr.setDate(params.get("checkout"));
  }
  if (params.has("sort")) {
  ...
  if (rawAmenities) {
    ...
    document
      .querySelectorAll(".amenities-check-grid input[type='checkbox']")
      .forEach((cb) => {
        cb.checked = selected.has(cb.value);
      });
  }
  ...
  if (searchApplyBtn) {
    searchApplyBtn.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".amenities-check-grid input:checked")
        .forEach((cb) => {
