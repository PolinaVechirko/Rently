(function createSearchPage(window) {
  const advancedFilters = window.RentlySearchAdvancedFilters;
  const dateRange = window.RentlySearchDateRange;
  const locationAutocomplete = window.RentlySearchLocationAutocomplete;
  const priceFilters = window.RentlySearchPriceFilters;
  const queryState = window.RentlySearchQueryState;
  ...
  function initSearchPage() {
    ...
    const params = queryState.getSearchParams();
    const pageElements = {
    ...
    searchApplyButton?.addEventListener("click", (event) => {
      ...
      const nextParams = queryState.buildSearchQuery({
        location: locationInput.value,
        checkin: dateRange.formatDateForQuery(selectedDates.checkin),
        checkout: dateRange.formatDateForQuery(selectedDates.checkout),
        ...
        amenities: advancedFilters.getCheckedValues(
          ".amenities-check-grid input:checked",
        ),
        types: advancedFilters.getCheckedValues(
